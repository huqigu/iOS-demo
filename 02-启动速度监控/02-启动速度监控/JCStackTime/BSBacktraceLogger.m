...
+ (NSArray *)bs_backtraceMapArrayOfMainThread {
    return _bs_backtraceMapArrayOfThread(bs_machThreadFromNSThread([NSThread mainThread]));
}
...
NSArray *_bs_backtraceMapArrayOfThread(thread_t thread) {
    uintptr_t backtraceBuffer[50];
    int i = 0;
//    NSMutableString *resultString = [[NSMutableString alloc] initWithFormat:@"Backtrace of Thread %u:\n", thread];
...
//        return @"Fail to get frame pointer";
    ...
    for (int i = 0; i < backtraceLength; ++i) {
//        [resultString appendFormat:@"%@", bs_logBacktraceEntry(i, backtraceBuffer[i], &symbolicated[i])];
        [mArray addObject:bs_logBacktraceMapModelEntry(i, backtraceBuffer[i], &symbolicated[i])];
    }
    return mArray;
//    [resultString appendFormat:@"\n"];
//    return [resultString copy];
...
JCStackMethodModel* bs_logBacktraceMapModelEntry(const int entryNum,
                               const uintptr_t address,
                               const Dl_info* const dlInfo) {
    char faddrBuff[20];
    char saddrBuff[20];
    ...
    JCStackMethodModel *model = [[JCStackMethodModel alloc] init];
//    model.time = 0.0f;
    model.methodName = [NSString stringWithFormat:@"%s",sname];
    model.methodAddress = [NSString stringWithFormat:@"0x%08" PRIxPTR,(uintptr_t)address];
    return model;
//    return [NSString stringWithFormat:@"%-30s  0x%08" PRIxPTR " %s + %lu\n" ,fname, (uintptr_t)address, sname, offset];
}
